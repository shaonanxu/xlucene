...
/**
 * <p>
 * Allows multiple {@link Filter}s to be chained. Logical operations such as
 * <b>NOT</b> and <b>XOR</b> are applied between filters. One operation can be
 * used for all filters, or a specific operation can be declared for each
 * filter.
 * </p>
 * <p>
 * Order in which filters are called depends on the position of the filter in
 * the chain. It's probably more efficient to place the most restrictive filters
...
public class ChainedFilter extends Filter {
	...
	private FixedBitSet initialResult(AtomicReaderContext context, int logic, int[] index) throws IOException {
		...
		if (logic == AND) {
			...
			result.or(getDISI(chain[index[0]], context));
			result.flip(0, reader.maxDoc()); // NOTE: may set bits for deleted
												// docs.
			++index[0];
		}
		return result;
