...
final class DefaultIndexingChain extends DocConsumer {
   ...
   */
    ...
    // How many indexed field names we've seen (collapses multiple field instances by the same name):
		int fieldCount = 0;
    ...
    // (i.e., we cannot have more than one TokenStream running "at once"):
  ...
  private static void verifyUnIndexedFieldType(String name, IndexableFieldType ft) {
	...
	private void indexDocValue(PerField fp, DocValuesType dvType, IndexableField field) throws IOException {
      ...
      // This will throw an exc if the caller tried to change the DV type for the field:
