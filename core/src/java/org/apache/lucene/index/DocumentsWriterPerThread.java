...
class DocumentsWriterPerThread {
  ...
  public void updateDocument(Iterable<? extends IndexableField> doc, Analyzer analyzer, Term delTerm) throws IOException {
    ...
    assert deleteQueue != null;
        ...
        // Aborting exceptions will actually "lose" more than one
        // document, so the counter will be "wrong" in that case, but
        // it's very hard to fix (we can't easily distinguish aborting vs non-aborting exceptions):
    			reserveDoc();
    			docState.doc = doc;
    			docState.docID = numDocsInRAM;
    			docCount++;
            ...
            // An exc is being thrown...
						if (!aborting) {
              // Incr here because finishDocument will not be called (because an exc is being thrown):
							numDocsInRAM++;
						} else {
							abort(filesToDelete);
						}
  ...
  /** Flush all pending docs to a new segment */
    ...
    segmentInfo.setDocCount(numDocsInRAM);
		final SegmentWriteState flushState = 
				new SegmentWriteState(infoStream, directory, segmentInfo, fieldInfos.finish(), indexWriterConfig.getTermIndexInterval(),
