...
class FieldCacheImpl implements FieldCache {
  ...
  public Ints getInts (AtomicReader reader, String field, boolean setDocsWithField) throws IOException {
	...
	public Ints getInts(AtomicReader reader, String field, IntParser parser, boolean setDocsWithField) throws IOException {
		final NumericDocValues valuesIn = reader.getNumericDocValues(field);
		if (valuesIn != null) {
      // Not cached here by FieldCacheImpl (cached instead per-thread by SegmentReader):
			return new Ints() {
				@Override
  ...
  static final class IntCache extends Cache {
    ...
    @Override
    protected Accountable createValue(final AtomicReader reader, CacheKey key, boolean setDocsWithField) throws IOException {

      final IntParser parser = (IntParser) key.custom;
      if (parser == null) {
        // Confusing: must delegate to wrapper (vs simply setting parser = DEFAULT_INT_PARSER/NUMERIC_UTILS_INT_PARSER)
        // so cache key includes DEFAULT_INT_PARSER/NUMERIC_UTILS_INT_PARSER:
        try {
  ...
  // TODO: this if DocTermsIndex was already created, we should share it...
	...
	public BinaryDocValues getTerms(AtomicReader reader, String field, boolean setDocsWithField, float acceptableOverheadRatio) throws IOException {
      ...
      // per-thread by SegmentReader):
		...
		BinaryDocValuesImpl impl = 
				(BinaryDocValuesImpl) caches.get(BinaryDocValues.class).get(reader, new CacheKey(field, acceptableOverheadRatio), setDocsWithField);
		return impl.iterator();
	}
	...
	static final class BinaryDocValuesCache extends Cache {
		...
		@Override
		protected Accountable createValue(AtomicReader reader, CacheKey key, boolean setDocsWithField) throws IOException {
