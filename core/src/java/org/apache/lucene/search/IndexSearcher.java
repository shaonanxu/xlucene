...
public class IndexSearcher {
   ...
   *         {@link BooleanQuery#getMaxClauseCount()} clauses.
   */
	protected TopFieldDocs search(Weight weight, final int nDocs, Sort sort, boolean doDocScores, boolean doMaxScore) throws IOException {
   ...
   */
	protected TopFieldDocs search(Weight weight, FieldDoc after, int nDocs, Sort sort, boolean fillFields, 
			boolean doDocScores, boolean doMaxScore) throws IOException {
    ...
    nDocs = Math.min(nDocs, limit);
      ...
      // use all leaves here!
			...
			final TopFieldCollector topCollector = 
					TopFieldCollector.create(sort, nDocs, after, fillFields, doDocScores, doMaxScore, false);
   ...
   */
    ...
    // single thread
		...
		TopFieldCollector collector = 
				TopFieldCollector.create(sort, nDocs, after, fillFields, doDocScores, doMaxScore, !weight.scoresDocsOutOfOrder());
		search(leaves, weight, collector);
   ...
   */
    ...
    // always use single thread:
        ...
        // there is no doc of interest in this reader context continue with the following leaf
				continue;
			}
